...
def plot_add_method_comparison(sim_baseline, sim_intervention, new_method_name=None, intro_year=None, figsize=(10, 4), legend_kwargs=None):
    """
    Plot baseline vs add_method intervention: mCPR over time and (optionally) new method usage.

    Useful for showcasing the impact of introducing a new contraceptive method with the
    add_method intervention.

    Parameters
    ----------
    sim_baseline : Sim
        Simulation without the add_method intervention.
    sim_intervention : Sim
        Simulation with the add_method intervention.
    new_method_name : str, optional
        Name of the new method (as in the model) to plot usage over time. If None, only mCPR is plotted in the second panel.
    intro_year : float, optional
        Year when the new method was introduced; a vertical line is drawn at this year.
    ...
    -------
    fig : Figure
        The matplotlib figure (with two axes: mCPR comparison and new method usage).
    """
    if legend_kwargs is None:
        legend_kwargs = {'frameon': frameon, 'loc': 'upper left'}

    res_int = sim_intervention.results
    years = np.array([float(t) for t in res_int['timevec'].to_float()])
    mcpr_baseline = sim_baseline.results.contraception.mcpr * 100
    mcpr_int = res_int.contraception.mcpr * 100

    fig, axes = pl.subplots(1, 2, figsize=figsize)

    # Left: mCPR comparison
    ax = axes[0]
    ax.plot(years, mcpr_baseline, 'b-', lw=2, label=sim_baseline.label or 'Baseline', alpha=0.8)
    ax.plot(years, mcpr_int, 'r-', lw=2, label=sim_intervention.label or 'With new method', alpha=0.8)
    ...
    # Right: new method usage or placeholder
    ax = axes[1]
    if new_method_name:
        cm = sim_intervention.connectors.contraception
        fp_mod = sim_intervention.connectors.fp
        if new_method_name in cm.methods:
            idx = cm.methods[new_method_name].idx
            usage = fp_mod.method_mix[idx, :] * 100
            ax.plot(years, usage, 'g-', lw=2, label=new_method_name)
    if intro_year is not None:
        ax.axvline(x=intro_year, color='green', linestyle='--', alpha=0.7)
    ax.set_xlabel('Year')
    ax.set_ylabel('Usage (%)')
    ax.set_title('New method usage' if new_method_name else 'Method mix')
    ax.legend(**legend_kwargs)
    ax.grid(True, alpha=0.3)

    pl.tight_layout()
    if Config.do_save:
        save_figure('add_method_comparison.png')
    if Config.do_show:
        pl.show()
    return fig
